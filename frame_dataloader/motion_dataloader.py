...
class MotionSequence(keras.utils.Sequence):
    ...
    def __getitem__(self, batch_start):
        ...
        for vid_id, _ in enumerate(batch_y):  # for each sample here

            if self.is_training:  # max frame is given
                first_optical_frame_id = random.randint(1, batch_frames[vid_id])  # random frame (one based)
            else:
                first_optical_frame_id = batch_frames[vid_id]  # just as selected

            batch_x.append(  # append one sample which is (h,w,stacked*2)
                stack_opticalflow(start_frame_index=first_optical_frame_id, video_name=batch_video_names[vid_id], data_root_path=self.data_root_path, stacked_frames=self.stacked_frames)
            )
